...
class SyllabusController {
    ...
    public function index() {
        ...
        // Scope filter (berdasarkan major kelas)
        [$sw, $sp] = ScopeFilter::apply('c');
        if ($sw) { $where .= str_replace(' AND ', ' AND ', $sw); $params = array_merge($params, $sp); }
        ...
        if ($classroomId)        { $where .= " AND sd.classroom_id = ?"; $params[] = $classroomId; }

        $baseJoin = "FROM syllabus_documents sd
                     JOIN subjects s ON sd.subject_id = s.id
                     JOIN classrooms c ON sd.classroom_id = c.id
                     JOIN academic_years ay ON sd.academic_year_id = ay.id
                     JOIN users u ON sd.teacher_id = u.id";

        $totalData  = $db->query("SELECT COUNT(*) $baseJoin WHERE $where", $params)->fetchColumn();
        $totalPages = ceil($totalData / $limit);

        $documents = $db->query("SELECT sd.*, s.name as subject_name, ay.name as year_name,
                                        u.name as teacher_name, c.name as classroom_name, c.level
                                 $baseJoin WHERE $where ORDER BY sd.created_at DESC LIMIT $limit OFFSET $offset", $params)->fetchAll();
        ...
        [$cw, $cp] = ScopeFilter::apply('c');
        $classrooms = $db->query("SELECT * FROM classrooms c WHERE 1=1 $cw ORDER BY level, name", $cp)->fetchAll();
    ...
    public function store() {
        ...
        if (isset($_FILES['file']) && $_FILES['file']['error'] == 0) {
            ...
            if (move_uploaded_file($_FILES['file']['tmp_name'], $uploadDir . $fileName)) {
                $db->query("INSERT INTO syllabus_documents (teacher_id, subject_id, academic_year_id, classroom_id, type, title, file_path)
                            VALUES (?, ?, ?, ?, ?, ?, ?)", [
