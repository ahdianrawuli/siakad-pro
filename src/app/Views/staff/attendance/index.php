...
<main class="flex-1 overflow-y-auto bg-slate-50 p-4 md:p-6">
    ...
    <div class="mb-8 flex flex-col md:flex-row md:justify-between md:items-end gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div>
            <h3 class="text-2xl md:text-3xl font-extrabold text-slate-800 tracking-tight"><?= htmlspecialchars($title) ?></h3>
            <p class="text-slate-500 text-sm mt-1 font-medium">Pencatatan kehadiran harian pegawai.</p>
    ...
    <div class="flex flex-col gap-6">
        ...
        <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
            <form method="GET" class="flex flex-wrap items-center gap-3">
                ...
                <div class="flex-1 min-w-[200px] relative">
                    ...
                    <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Cari nama pegawai..."
                ...
                <?php if (!empty($search)): ?>
                    <a href="?date=<?= $date ?>" class="flex items-center justify-center w-10 h-10 bg-red-50 text-red-500 rounded-xl hover:bg-red-100 transition" title="Reset">
        ...
        <!-- Table Card -->
        <form action="/attendance/staff/store" method="POST">
            <?= \App\Core\Csrf::input() ?>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            ...
                            <tr class="hover:bg-slate-50/80 transition-colors text-sm">
                                <td class="px-5 py-4">
                                    ...
                                    <div class="text-[10px] text-slate-400 font-mono mt-0.5 uppercase"><?= $u['role_slug'] ?></div>
                                </td>
                                ...
                                <td class="px-5 py-4">
                                    <input type="text" name="notes[<?= $u['id'] ?>]" value="<?= htmlspecialchars($att['notes'] ?? '') ?>"
                                        class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:bg-white focus:ring-2 focus:ring-blue-500/50 outline-none transition-all"
                                        placeholder="Keterangan...">
                                </td>
                                <td class="px-5 py-4 text-center">
                                    <?php if ($att): ?>
                                        <a href="/attendance/staff/delete?user_id=<?= $u['id'] ?>&date=<?= $date ?>"
                                            onclick="return confirm('Reset absensi orang ini?')"
                                            class="w-8 h-8 rounded-lg bg-red-50 text-red-500 hover:bg-red-500 hover:text-white inline-flex items-center justify-center transition-colors shadow-sm" title="Reset">
                ...
                <div class="p-4 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="flex items-center gap-3">
                        ...
                        <button type="submit" class="bg-blue-600 text-white px-5 py-2 rounded-xl text-sm font-bold shadow-md shadow-blue-500/20 hover:bg-blue-700 transition-all flex items-center gap-2">
...
<div id="infoModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        <div class="p-5 border-b border-slate-100 bg-blue-50 flex justify-between items-center">
            <h3 class="font-bold text-blue-800 flex items-center gap-2"><i class="fa-solid fa-circle-info text-blue-500"></i> Panduan Absensi Pegawai</h3>
        ...
        <div class="p-6 space-y-5 text-sm text-slate-600">
            <div>
                ...
                <ol class="list-decimal list-inside space-y-1.5 text-slate-500">
                    <li>Pilih <strong class="text-slate-700">tanggal</strong> yang ingin dicatat absensinya.</li>
                    <li>Pilih status kehadiran untuk setiap pegawai: <strong class="text-slate-700">Hadir, Izin, Sakit, atau Alpa</strong>.</li>
                    <li>Isi kolom <strong class="text-slate-700">Catatan</strong> jika diperlukan keterangan tambahan.</li>
            ...
            <div>
                ...
                <div class="space-y-2">
                    <div class="flex items-center gap-3 p-2.5 bg-slate-50 rounded-xl border border-slate-200">
                        ...
                        <div><div class="font-semibold text-slate-700 text-xs">Data Pegawai</div><div class="text-[11px] text-slate-400">Daftar pegawai diambil dari <strong>Kepegawaian → Data Pegawai</strong>.</div></div>
                    </div>
                    <div class="flex items-center gap-3 p-2.5 bg-slate-50 rounded-xl border border-slate-200">
                        <i class="fa-solid fa-sitemap text-purple-400 w-5 text-center"></i>
                        <div><div class="font-semibold text-slate-700 text-xs">Master Jabatan</div><div class="text-[11px] text-slate-400">Jabatan pegawai dikelola di <strong>Kepegawaian → Master Jabatan</strong>.</div></div>
                    </div>
                    <div class="flex items-center gap-3 p-2.5 bg-slate-50 rounded-xl border border-slate-200">
                        <i class="fa-solid fa-chart-bar text-green-400 w-5 text-center"></i>
                        <div><div class="font-semibold text-slate-700 text-xs">Rekap Absensi</div><div class="text-[11px] text-slate-400">Laporan kehadiran bulanan tersedia di menu <strong>Laporan → Rekap Absensi</strong>.</div></div>
                    </div>
                    <div class="flex items-center gap-3 p-2.5 bg-slate-50 rounded-xl border border-slate-200">
                        <i class="fa-solid fa-rotate-left text-red-400 w-5 text-center"></i>
                        <div><div class="font-semibold text-slate-700 text-xs">Reset Absensi</div><div class="text-[11px] text-slate-400">Klik ikon <strong>reset</strong> pada baris pegawai untuk menghapus absensi hari itu.</div></div>
                    </div>
                </div>
            </div>
        </div>
