...
<main class="flex-1 overflow-y-auto bg-slate-50 p-4 md:p-6">
    ...
    <div class="mb-8 flex flex-col md:flex-row md:justify-between md:items-end gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div>
            <h3 class="text-2xl md:text-3xl font-extrabold text-slate-800 tracking-tight">Master Jenis Tagihan</h3>
            <p class="text-slate-500 text-sm mt-1 font-medium">Kelola daftar jenis tagihan dan nominal default.</p>
            <div class="mt-3 flex items-center gap-2">
                <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg text-xs font-bold border border-blue-100">
                    <i class="fa-solid fa-file-invoice-dollar"></i> Total Jenis: <?= $totalData ?>
                </div>
        ...
        <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
            <form method="GET" class="flex flex-wrap items-center gap-3">
                ...
                <div class="flex-1 min-w-[220px] relative">
                    ...
                    <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Cari nama tagihan..."
                ...
                <?php if (!empty($search)): ?>
                    <a href="/finance/fee-types" class="flex items-center justify-center w-10 h-10 bg-red-50 text-red-500 rounded-xl hover:bg-red-100 transition" title="Reset">
                ...
                <table class="min-w-full whitespace-nowrap text-left">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-200">
                            <th class="px-5 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Nama Tagihan</th>
                            <th class="px-5 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Nominal Default</th>
                            <th class="px-5 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 bg-white">
                        <?php if (empty($types)): ?>
                            <tr><td colspan="3" class="px-5 py-16 text-center text-slate-400 text-sm font-medium">Belum ada data jenis tagihan.</td></tr>
                        <?php endif; ?>
                        ...
                        <tr class="hover:bg-slate-50/80 transition-colors text-sm">
                            ...
                            <td class="px-5 py-4 font-mono font-bold text-slate-700">Rp <?= number_format($t['amount'], 0, ',', '.') ?></td>
                            <td class="px-5 py-4 text-center">
                                <a href="/finance/fee-types/delete?id=<?= $t['id'] ?>"
...
<div id="infoModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        <div class="p-5 border-b border-slate-100 bg-blue-50 flex justify-between items-center">
            <h3 class="font-bold text-blue-800 flex items-center gap-2"><i class="fa-solid fa-circle-info text-blue-500"></i> Panduan Jenis Tagihan</h3>
        ...
        <div class="p-6 space-y-5 text-sm text-slate-600">
            <div>
                ...
                <ol class="list-decimal list-inside space-y-1.5 text-slate-500">
                    <li>Klik <strong class="text-slate-700">Tambah Baru</strong> untuk mendaftarkan jenis tagihan.</li>
                    <li>Isi nama tagihan dan nominal default.</li>
                    <li>Nominal default digunakan saat membuat tagihan massal.</li>
                </ol>
            </div>
            <div>
                ...
                <div class="space-y-2">
                    <div class="flex items-center gap-3 p-2.5 bg-slate-50 rounded-xl border border-slate-200">
                        ...
                        <div><div class="font-semibold text-slate-700 text-xs">Kasir / Tagihan Siswa</div><div class="text-[11px] text-slate-400">Jenis tagihan ini digunakan saat membuat tagihan di <strong>Keuangan → Kasir</strong>.</div></div>
                    </div>
                    <div class="flex items-center gap-3 p-2.5 bg-slate-50 rounded-xl border border-slate-200">
                        <i class="fa-solid fa-chart-bar text-green-400 w-5 text-center"></i>
                        <div><div class="font-semibold text-slate-700 text-xs">Laporan Keuangan</div><div class="text-[11px] text-slate-400">Rekap per jenis tagihan tersedia di <strong>Keuangan → Laporan</strong>.</div></div>
                    </div>
                </div>
            </div>
        </div>
...
<div id="addModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        <div class="p-5 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
            <h3 class="font-bold text-slate-700 flex items-center gap-2"><i class="fa-solid fa-file-invoice-dollar text-slate-400"></i> Tambah Jenis Tagihan</h3>
        ...
        <form action="/finance/fee-types/store" method="POST" class="p-6 space-y-4">
            <div>
                <label class="block text-sm font-semibold text-slate-600 mb-1.5">Nama Tagihan</label>
                <input type="text" name="name" placeholder="cth: SPP Bulanan, Uang Gedung"
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-600 mb-1.5">Nominal Default (Rp)</label>
