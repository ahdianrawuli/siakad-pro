...
<main class="flex-1 overflow-y-auto bg-slate-50 p-4 md:p-6">
    ...
    <div class="mb-8 flex flex-col md:flex-row md:justify-between md:items-end gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div>
            <h3 class="text-2xl md:text-3xl font-extrabold text-slate-800 tracking-tight">Prestasi Siswa</h3>
            <p class="text-slate-500 text-sm mt-1 font-medium">Rekam jejak pencapaian dan penghargaan siswa.</p>
            <div class="mt-3 flex items-center gap-2">
                <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-yellow-50 text-yellow-700 rounded-lg text-xs font-bold border border-yellow-100">
                    <i class="fa-solid fa-trophy"></i> Total Prestasi: <?= $totalData ?>
                </div>
                ...
                <table class="min-w-full whitespace-nowrap text-left">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-200">
                            ...
                            <th class="px-5 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Nama Prestasi</th>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        ...
                        <tr><td colspan="6" class="px-5 py-16 text-center text-slate-400 text-sm font-medium">Belum ada data prestasi.</td></tr>
                        <?php endif; ?>

                        <?php foreach ($achievements as $row): ?>
                        <tr class="hover:bg-yellow-50/20 transition-colors text-sm">
                            ...
                            <td class="px-5 py-4">
                                <div class="font-bold text-yellow-600 text-sm"><?= $row['title'] ?></div>
                                <div class="text-[10px] text-slate-400 italic max-w-xs truncate"><?= $row['description'] ?: '-' ?></div>
                            </td>
                            <td class="px-5 py-4">
                                <span class="px-2.5 py-1 text-[10px] font-bold rounded-lg bg-blue-50 text-blue-700 border border-blue-200"><?= $row['level'] ?></span>
                            </td>
                            <td class="px-5 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button onclick="openEditModal(this)"
                                        data-id="<?= $row['id'] ?>" data-student="<?= $row['student_id'] ?>"
                                        data-title="<?= htmlspecialchars($row['title']) ?>" data-level="<?= $row['level'] ?>"
                                        data-date="<?= $row['date'] ?>" data-desc="<?= htmlspecialchars($row['description'] ?? '') ?>"
                                    ...
                                    <a href="/student-affairs/achievements/delete?id=<?= $row['id'] ?>"
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        <div class="p-5 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
            <h3 class="font-bold text-slate-700 flex items-center gap-2">
                <i class="fa-solid fa-trophy text-yellow-500"></i> Catat Prestasi Baru
            </h3>
        ...
        <form action="/student-affairs/achievements/store" method="POST" class="p-6 space-y-4">
            <?= \App\Core\Csrf::input() ?>
            <div>
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-600 mb-1.5">Nama Prestasi / Juara</label>
                <input type="text" name="title" placeholder="cth: Juara 1 Lomba Pidato Bahasa Arab"
                    class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-yellow-400/50 outline-none transition-all" required>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="2" placeholder="cth: Diselenggarakan oleh Kemenag Provinsi Sumatera Barat"
                    class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-yellow-400/50 outline-none transition-all resize-none"></textarea>
            </div>
            <div class="flex gap-3 pt-2">
                ...
                <button type="submit" class="flex-1 bg-yellow-500 text-white py-2.5 rounded-xl font-bold hover:bg-yellow-600 shadow-md shadow-yellow-500/20 transition text-sm">Simpan</button>
            </div>
        </form>
    </div>
</div>
...
<div id="editModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        <div class="p-5 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
            <h3 class="font-bold text-slate-700 flex items-center gap-2">
                <i class="fa-solid fa-pen-to-square text-slate-400"></i> Edit Data Prestasi
            </h3>
        ...
        <form action="/student-affairs/achievements/update" method="POST" class="p-6 space-y-4">
            <?= \App\Core\Csrf::input() ?>
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-600 mb-1.5">Nama Prestasi</label>
                <input type="text" name="title" id="edit_title" placeholder="cth: Juara 1 Lomba Pidato"
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-semibold text-slate-600 mb-1.5">Tingkat</label>
                    <select name="level" id="edit_level" class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white outline-none">
                        <option value="SEKOLAH">Sekolah</option>
                        <option value="KECAMATAN">Kecamatan</option>
                        <option value="KABUPATEN">Kabupaten/Kota</option>
                        <option value="PROVINSI">Provinsi</option>
            ...
            <div>
                ...
                <textarea name="description" id="edit_desc" rows="2" placeholder="cth: Diselenggarakan oleh Kemenag Provinsi"
...
<div id="infoModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        <div class="p-5 border-b border-slate-100 bg-blue-50 flex justify-between items-center">
            <h3 class="font-bold text-blue-800 flex items-center gap-2"><i class="fa-solid fa-circle-info text-blue-500"></i> Panduan Prestasi Santri</h3>
        ...
        <div class="p-6 space-y-5 text-sm text-slate-600">
            <div>
                ...
                <ol class="list-decimal list-inside space-y-1.5 text-slate-500">
                    <li>Klik <strong class="text-slate-700">Tambah Prestasi</strong> untuk mencatat pencapaian santri.</li>
                    <li>Isi nama lomba, tingkat (sekolah/kota/provinsi/nasional), dan peringkat.</li>
                    <li>Gunakan filter untuk menyaring berdasarkan santri atau tingkat prestasi.</li>
                </ol>
            </div>
            <div>
                ...
                <div class="space-y-2">
                    <div class="flex items-center gap-3 p-2.5 bg-slate-50 rounded-xl border border-slate-200">
                        ...
                        <div><div class="font-semibold text-slate-700 text-xs">Data Santri</div><div class="text-[11px] text-slate-400">Prestasi terhubung dengan santri di <strong>Kesiswaan → Data Santri</strong>.</div></div>
                    </div>
                    <div class="flex items-center gap-3 p-2.5 bg-slate-50 rounded-xl border border-slate-200">
                        <i class="fa-solid fa-route text-green-400 w-5 text-center"></i>
                        <div><div class="font-semibold text-slate-700 text-xs">Pelacakan Santri</div><div class="text-[11px] text-slate-400">Riwayat prestasi tampil di <strong>Kesiswaan → Pelacakan Santri</strong>.</div></div>
                    </div>
                </div>
            </div>
        </div>
