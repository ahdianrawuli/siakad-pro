...
<main class="flex-1 overflow-y-auto bg-gray-100 p-6">
    <div class="mb-6 flex justify-between items-end">
        <div>
            <h3 class="text-3xl font-medium text-gray-700">Prestasi Siswa</h3>
            <p class="text-gray-500 text-sm">Rekam jejak pencapaian dan penghargaan siswa.</p>
        </div>
        
        <button onclick="document.getElementById('addModal').classList.remove('hidden')" 
           class="bg-yellow-500 text-white px-4 py-2 rounded-lg font-bold hover:bg-yellow-600 shadow-lg transition text-sm flex items-center">
            <i class="fa-solid fa-trophy mr-2"></i> Tambah Prestasi
        </button>
    </div>
    ...
    <div class="space-y-4">
            ...
            <form method="GET" class="flex flex-wrap gap-3">
                ...
                <div class="flex-1 min-w-[300px] relative">
                    ...
                    <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" 
                           placeholder="Cari Nama Siswa atau Nama Prestasi..." 
                           class="w-full pl-10 pr-4 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-yellow-400 outline-none">
                </div>
                ...
                <?php if(!empty($search)): ?>
                    <a href="/student-affairs/achievements" class="px-4 py-2 bg-red-100 text-red-600 rounded-lg text-sm font-bold hover:bg-red-200">Reset</a>
                <?php endif; ?>
            </form>
        </div>
        ...
        <div class="bg-white rounded shadow-sm border border-gray-200 overflow-hidden flex flex-col">
            <div class="p-4 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                <h4 class="font-bold text-gray-700 text-xs uppercase">Daftar Prestasi</h4>
                ...
                <table class="min-w-full whitespace-nowrap">
                    <thead>
                        <tr class="bg-gray-100 text-left text-[11px] font-bold text-gray-500 uppercase tracking-wider">
                            ...
                            <th class="px-5 py-4 border-b">Nama Prestasi</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr>
                            <td colspan="6" class="px-5 py-12 text-center text-gray-400 italic text-sm">Belum ada data prestasi.</td>
                        </tr>
                        <?php endif; ?>

                        <?php foreach ($achievements as $row): ?>
                        <tr class="hover:bg-yellow-50/30 transition text-sm">
                            ...
                            <td class="px-5 py-4">
                                ...
                                <div class="text-[10px] text-gray-500 italic max-w-xs truncate"><?= $row['description'] ?: '-' ?></div>
                            </td>
                            <td class="px-5 py-4">
                                <span class="px-2 py-1 text-[10px] font-bold rounded bg-blue-50 text-blue-600 border border-blue-100">
                                    <?= $row['level'] ?>
                                </span>
                            </td>
            ...
            <div class="p-4 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
                ...
                <div class="flex gap-1">
                    ...
                    <?php for($i = 1; $i <= $totalPages; $i++): ?>
                        <a href="?page=<?= $i . $queryString ?>" class="px-3 py-1 border rounded text-xs transition <?= $i == $currentPage ? 'bg-yellow-500 text-white border-yellow-500 font-bold' : 'bg-white hover:bg-gray-100' ?>">
                            <?= $i ?>
                        </a>
                    <?php endfor; ?>
                </div>
    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate__animated animate__zoomIn animate__faster">
        <div class="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
            <h3 class="font-bold text-gray-700 flex items-center">
                <i class="fa-solid fa-trophy mr-2 text-yellow-500"></i> Catat Prestasi Baru
            </h3>
            <button onclick="document.getElementById('addModal').classList.add('hidden')" class="text-gray-400 hover:text-gray-600">
                <i class="fa-solid fa-xmark text-xl"></i>
            </button>
        </div>
        <form action="/student-affairs/achievements/store" method="POST" class="p-6">
            <?= \App\Core\Csrf::input() ?>
            
            <div class="mb-4">
                <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Nama Siswa</label>
                <select name="student_id" class="w-full p-2.5 border rounded-lg text-sm bg-white focus:ring-2 focus:ring-yellow-400 outline-none" required>
            ...
            <div class="mb-4">
                <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Nama Prestasi / Juara</label>
                <input type="text" name="title" placeholder="Contoh: Juara 1 Lomba Pidato" class="w-full p-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-yellow-400 outline-none" required>
            </div>

            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Tingkat</label>
                    <select name="level" class="w-full p-2.5 border rounded-lg text-sm bg-white focus:ring-2 focus:ring-yellow-400 outline-none" required>
                ...
                <div>
                    ...
                    <input type="date" name="date" value="<?= date('Y-m-d') ?>" class="w-full p-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-yellow-400 outline-none" required>
                </div>
            </div>

            <div class="mb-6">
                <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Keterangan Tambahan</label>
                <textarea name="description" rows="2" placeholder="Detail penyelenggara atau keterangan lain..." class="w-full p-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-yellow-400 outline-none"></textarea>
            </div>
            ...
            <div class="flex gap-3">
                ...
                <button type="submit" class="flex-1 bg-yellow-500 text-white py-3 rounded-lg font-bold hover:bg-yellow-600 shadow-lg transition">Simpan</button>
            </div>
        </form>
    </div>
</div>
...
<div id="editModal" class="hidden fixed inset-0 z-50 overflow-auto bg-black bg-opacity-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate__animated animate__zoomIn animate__faster">
        <div class="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
            <h3 class="font-bold text-gray-700 flex items-center">
                <i class="fa-solid fa-pen-to-square mr-2 text-blue-500"></i> Edit Data Prestasi
            </h3>
            <button onclick="document.getElementById('editModal').classList.add('hidden')" class="text-gray-400 hover:text-gray-600">
                <i class="fa-solid fa-xmark text-xl"></i>
            </button>
        </div>
        <form action="/student-affairs/achievements/update" method="POST" class="p-6">
            <?= \App\Core\Csrf::input() ?>
            ...
            <div class="mb-4">
                <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Nama Prestasi</label>
                <input type="text" name="title" id="edit_title" class="w-full p-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-blue-400 outline-none" required>
            </div>

            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Tingkat</label>
                    <select name="level" id="edit_level" class="w-full p-2.5 border rounded-lg text-sm bg-white focus:ring-2 focus:ring-blue-400 outline-none" required>
                        <option value="SEKOLAH">Sekolah</option>
                        <option value="KECAMATAN">Kecamatan</option>
                        <option value="KABUPATEN">Kabupaten/Kota</option>
                        <option value="PROVINSI">Provinsi</option>
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="edit_desc" rows="2" class="w-full p-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-blue-400 outline-none"></textarea>
            </div>
