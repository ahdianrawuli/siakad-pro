...
<body class="bg-white p-8 text-sm text-gray-800">
    ...
    <div class="text-center mb-6 border-b-2 border-gray-800 pb-4">
        <h1 class="text-xl font-bold uppercase">Rekap Prestasi Siswa</h1>
    ...
    <table class="w-full border-collapse border border-gray-300 text-left">
        <thead>
            <tr class="bg-gray-100 text-xs uppercase">
                ...
                <th class="border border-gray-300 px-3 py-2">Prestasi</th>
        ...
        <tbody>
            ...
            <?php endif; ?>
            <?php $no=1; foreach ($achievements as $a): ?>
            <tr class="<?= $no%2===0?'bg-gray-50':'' ?>">
                <td class="border border-gray-300 px-3 py-2 text-center"><?= $no++ ?></td>
                <td class="border border-gray-300 px-3 py-2 font-mono text-xs"><?= date('d/m/Y', strtotime($a['date'])) ?></td>
                <td class="border border-gray-300 px-3 py-2 font-mono text-xs"><?= $a['nis'] ?></td>
                <td class="border border-gray-300 px-3 py-2 font-bold"><?= htmlspecialchars($a['full_name']) ?></td>
                <td class="border border-gray-300 px-3 py-2 text-xs"><?= htmlspecialchars($a['class_name'] ?? '-') ?></td>
                <td class="border border-gray-300 px-3 py-2"><?= htmlspecialchars($a['title']) ?></td>
                <td class="border border-gray-300 px-3 py-2 text-center text-xs font-bold"><?= $a['level'] ?></td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    ...
    <div class="mt-10 flex justify-end">
        <div class="text-center">
            <p class="mb-16">Mengetahui, Kepala Sekolah</p>
